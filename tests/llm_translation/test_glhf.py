...
from litellm.llms.glhf.chat.transformation import GLHFChatConfig, GLHF_API_BASE


def test_glhf_chat_config_get_openai_compatible_provider_info():
    config = GLHFChatConfig()

    # Test with default values
    api_base, api_key = config._get_openai_compatible_provider_info(
        api_base=None, api_key=None
    )
    assert api_base == GLHF_API_BASE
    assert api_key == os.environ.get("GLHF_API_KEY")
    ...
    with patch.dict(
        "os.environ",
        {"GLHF_API_BASE": "https://glhf.chat/api/openai/v1", "GLHF_API_KEY": "env_api_key"},
    ):
        api_base, api_key = config._get_openai_compatible_provider_info(None, None)
        assert api_base == "https://glhf.chat/api/openai/v1"
        assert api_key == "env_api_key"


def test_glhf_chat_config_map_openai_params():
    """
    GLHF.CHAT is OpenAI compatible*
...
def test_completion_glhf(stream):
    try:
